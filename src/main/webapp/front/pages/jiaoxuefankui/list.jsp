  ...
  <div id="app">
      ...
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="ziyuanbianhao" id="ziyuanbianhao" placeholder="资源编号" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
          <label class="layui-form-label">教学项目</label>
          <div class="layui-input-inline">
            <input type="text" name="jiaoxuexiangmu" id="jiaoxuexiangmu" placeholder="教学项目" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
          <label class="layui-form-label">学生账号</label>
          <div class="layui-input-inline">
            <input type="text" name="xueshengzhanghao" id="xueshengzhanghao" placeholder="学生账号" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="fankuibiaoti" id="fankuibiaoti" placeholder="反馈标题" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('jiaoxuefankui','新增')" @click="jump('../jiaoxuefankui/add.jsp')" type="button" class="layui-btn btn-theme">
    ...
    <div class="product">
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        ...
        <div class="proBoxtxt">
          <a :href="'../jiaoxuefankui/detail.jsp?id='+item.id">查看详情</a>
        </div>
      </div>
      <div class="clear"></div>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#ziyuanbianhao').val()) {
          param['ziyuanbianhao'] = jquery('#ziyuanbianhao').val() ? '%' + jquery('#ziyuanbianhao').val() + '%' : '';
        }  
        if (jquery('#jiaoxuexiangmu').val()) {
          param['jiaoxuexiangmu'] = jquery('#jiaoxuexiangmu').val() ? '%' + jquery('#jiaoxuexiangmu').val() + '%' : '';
        }  
        if (jquery('#xueshengzhanghao').val()) {
          param['xueshengzhanghao'] = jquery('#xueshengzhanghao').val() ? '%' + jquery('#xueshengzhanghao').val() + '%' : '';
        }  
        if (jquery('#xueshengxingming').val()) {
          param['xueshengxingming'] = jquery('#xueshengxingming').val() ? '%' + jquery('#xueshengxingming').val() + '%' : '';
        }  
        if (jquery('#fankuibiaoti').val()) {
          param['fankuibiaoti'] = jquery('#fankuibiaoti').val() ? '%' + jquery('#fankuibiaoti').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('jiaoxuefankui/list', 'get', param, function(res) {
