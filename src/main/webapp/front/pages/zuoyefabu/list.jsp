    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('zuoyefabu','新增')" @click="jump('../zuoyefabu/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        <div class="proBoximg">
          <div class="primg" style="background-size: cover;background-repeat: no-repeat;background-position: center;">
            <img :src="item.zuoyetupian?item.zuoyetupian.split(',')[0]:''" width="390" height="320" />
          </div>
        </div>
        <div class="proBoxtxt">
          <p>{{item.zuoyebianhao}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
          <p>{{item.zuoyebiaoti}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
          <a :href="'../zuoyefabu/detail.jsp?id='+item.id">查看详情</a>
        </div>
      </div>
      <div class="clear"></div>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#zuoyebianhao').val()) {
          param['zuoyebianhao'] = jquery('#zuoyebianhao').val() ? '%' + jquery('#zuoyebianhao').val() + '%' : '';
        }  
        ...
        // 获取列表数据
        http.request('zuoyefabu/list', 'get', param, function(res) {
