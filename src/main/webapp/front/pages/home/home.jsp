...
<body>
  <div id="app">
    ...
    <div class="part2" style="margin-top: 50px;">
      <div class="part2_1">
        ...
        <div class="more">
            <a href="#" @click="jump('../jiaoxueziyuan/list.jsp')">MORE>></a>
        </div>
      </div>
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in jiaoxueziyuanRecommend" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.jiaoxuetupian?item.jiaoxuetupian.split(',')[0]:''" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../jiaoxueziyuan/detail.jsp?id='+item.id">查看详情</a>
            </div>
          </div>
          <div class="caseBox_2" style="text-align: center;">
            <a :href="'../jiaoxueziyuan/detail.jsp?id='+item.id">{{item.ziyuanbianhao}}</a>
          </div>
          <div class="caseBox_2" style="text-align: center;">
            <a :href="'../jiaoxueziyuan/detail.jsp?id='+item.id">{{item.jiaoxuexiangmu}}</a>
          </div>
          <div class="caseBox_2" style="text-align: center;">
            <a :href="'../jiaoxueziyuan/detail.jsp?id='+item.id">{{item.jiaoshixingming}}</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery'], function() {
      ...
      http.request(`jiaoxueziyuan/autoSort`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
        vue.jiaoxueziyuanRecommend = res.data.list;
        vue.$nextTick(()=>{
